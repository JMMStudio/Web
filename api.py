if __name__ == '__main__':
    ...
    # --- MODIFICADO ---
    HOST = "0.0.0.0" # Cambiado de "127.0.0.1" a "0.0.0.0" para accesibilidad Docker
    # --- FIN MODIFICADO ---
    PORT = 9011
    if Path(ROOT_DIR+'/host.txt').is_file():
        ...
        if len(host_str)>0:
            HOST=host_str[0] # Se sobrescribirá si host.txt existe y es válido
        if len(host_str)==2:
            PORT=int(host_str[1]) # Se sobrescribirá si host.txt existe y es válido
    ...
    # url前缀 - Nota: Esto podría necesitar ajuste si se accede desde fuera con un host diferente
